      ...
      <main>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 items-center gap-6 px-6 sm:px-8 lg:px-10 pt-6 pb-7">
              ...
              <div className="flex justify-center lg:justify-end">
                <img
                  ...
                  className="w-full max-w-[540px] object-contain animate-[heroImage_.9s_ease-out] transition-transform duration-700 hover:scale-[1.04]"
                />
              </div>
        ...
        <section className="mx-auto max-w-[1400px] px-5 sm:px-8 lg:px-12 mt-14 sm:mt-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-7">
            {industryExpertise.industries.slice(0, 3).map((industry, index) => {
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-7 md:max-w-[920px] md:mx-auto">
            {industryExpertise.industries.slice(3, 5).map((industry, index) => {
              const Icon = industry.icon;
        ...
        <section className="mx-auto max-w-[1400px] px-5 sm:px-8 lg:px-12 mt-16 sm:mt-20 pb-10">
          <div className="group rounded-2xl bg-[#D9F8EB] px-6 sm:px-8 lg:px-10 py-7 sm:py-8 transition-all duration-300 hover:-translate-y-1 hover:bg-[#C8F2E0] hover:shadow-[0_10px_25px_rgba(0,86,65,0.12)]">

            <div className="flex items-center gap-5 sm:gap-6">

              {/* ICON */}
              <div className="flex h-14 w-14 sm:h-16 sm:w-16 shrink-0 items-center justify-center rounded-full bg-[#005641] transition-transform duration-300 group-hover:scale-105">
                <Globe2
                  size={29}
                  strokeWidth={1.7}
...
function IndustryCard({ industry, Icon, index }) {
  return (
    <div
      ...
      className="group animate-[fadeUp_.6s_ease-out_both] min-h-[300px] sm:min-h-[310px] rounded-xl border border-[#E5E8E6] bg-white px-6 sm:px-7 lg:px-8 py-7 sm:py-8 shadow-[0_4px_10px_rgba(0,0,0,0.10)] transition-all duration-300 hover:-translate-y-1 hover:bg-[#E2F8EF] hover:border-[#BFE6D4] hover:shadow-[0_12px_25px_rgba(0,86,65,0.13)]"
    >

      {/* ICON */}
      <div className="flex h-16 w-16 items-center justify-center rounded-full bg-[#D9F8EB] transition-all duration-300 group-hover:bg-white group-hover:scale-110">
        <Icon
          size={32}
          strokeWidth={1.5}
      ...
      {/* TITLE */}
      <h2 className="mt-7 text-[18px] sm:text-[19px] lg:text-[20px] font-bold leading-[1.3] text-[#101F1D]">
        {industry.title}
      </h2>
      ...
      {/* ITEMS */}
      <ul className="mt-6 space-y-2 text-[14px] sm:text-[15px] leading-[1.4] text-[#4F5958]">
        {industry.items.map((item) => (
          <li
            key={item}
            className="flex items-start gap-2"
          >
            <span className="mt-[7px] h-[5px] w-[5px] shrink-0 rounded-full bg-[#4A9877] transition-transform duration-300 group-hover:scale-125" />
            <span>{item}</span>
          </li>
        ))}
      </ul>
